...
密文一维批归一化（仅推理模式），对应 PyTorch `nn.BatchNorm1d`。

## 签名

```python
hetorch2.nn.BatchNorm1d(num_features, eps=1e-5, momentum=0.1, affine=True, track_running_stats=True, device='cpu')
```

## 参数

- `num_features`: 特征通道数 \(C\)（对输入形状 `(N, C)` 或 `(N, C, L)` 中的 `C`）。
- `eps`: 数值稳定项，加在方差上。
