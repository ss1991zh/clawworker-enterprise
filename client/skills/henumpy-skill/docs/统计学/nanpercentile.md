...
### 在忽略 NaN 值的情况下，沿着指定的轴计算数据的第$ q $个百分位数
...
标量密文 `nanpercentile(a,q)`：$ a $的第$ q $个百分位数，忽略 NaN 值
...
res = hp.nanpercentile(A, q)		
print(ct.decrypt(res))
# 输出 1.1000000000000003

# 数组, axis=0
res = hp.nanpercentile(A, q, axis=0)
print(ct.decrypt(res))
# 输出 [ 1.7  -1.6   3.35]
```
