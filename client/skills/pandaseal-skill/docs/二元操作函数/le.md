...
Element-wise **less-than-or-equal** comparison between a `CipherSeries` or `CipherDataFrame` and `other`. Equivalent to operators `<=` where supported.
...
mask = cipher_df.le(150)
# Decrypted truth table: cost F,T,T; revenue T,F,F
```
