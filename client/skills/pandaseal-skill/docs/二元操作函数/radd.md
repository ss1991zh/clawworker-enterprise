...
Reverse element-wise addition: `other + CipherSeries` or `other + CipherDataFrame`. Supports `fill_value` when either side has missing data.
...
- **other** — `CipherSeries`, `CipherArray`, or scalar: left-hand operand (added to the series).
...
`CipherSeries` or `CipherDataFrame`: element-wise sum (`other + self`).
...
out = cipher_df.radd(1)
# same decrypted result as add(1): angles +1, degrees +1
```
