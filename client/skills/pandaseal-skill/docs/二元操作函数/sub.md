...
Element-wise subtraction: left minus right for `CipherSeries` or `CipherDataFrame`. Same idea as `CipherSeries - other` / `CipherDataFrame - other`, with optional `fill_value` for missing values before alignment.
...
- **other** — `CipherSeries`, `CipherArray`, or scalar: subtracted from the series.
...
`CipherSeries` or `CipherDataFrame`: element-wise difference.
...
out = cipher_df.sub(1)
# ct.decrypt_df(out): angles -1,2,3; degrees 359,179,359
```
