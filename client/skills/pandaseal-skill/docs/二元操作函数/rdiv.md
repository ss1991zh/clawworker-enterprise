...
Reverse element-wise division: `other / CipherSeries` or `other / CipherDataFrame`. Supports `fill_value` when either side has missing data.
...
- **other** — `CipherSeries`, `CipherArray`, or scalar: numerator.
...
out = cipher_df.rdiv(360)
# ct.decrypt_df(out): 360 / angles, 360 / degrees per cell
```
