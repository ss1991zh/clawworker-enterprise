...
Element-wise **greater-than** comparison between a `CipherSeries` or `CipherDataFrame` and `other`. Equivalent to operators `>` where supported.
...
mask = cipher_df.gt(150)
# Decrypted truth table: cost T,F,F; revenue F,T,T
```
