...
Element-wise **not-equal** comparison between a `CipherSeries` or `CipherDataFrame` and `other`. Equivalent to operators `!=` where supported.
...
mask = cipher_df.ne(150)
# Decrypted truth table: cost T,F,T; revenue T,T,T (B cost equals 150)
```
