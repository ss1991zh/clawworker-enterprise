...
Element-wise multiplication of a `CipherSeries` or `CipherDataFrame` with another object. Same idea as `CipherSeries * other` / `CipherDataFrame * other`, with optional `fill_value` for missing values before alignment.
...
- **other** — `CipherSeries`, `CipherArray`, or scalar: multiplier.
...
`CipherSeries` or `CipherDataFrame`: element-wise product.
...
out = cipher_df.mul(2)
# ct.decrypt_df(out): angles 0,6,8; degrees 720,360,720
```
