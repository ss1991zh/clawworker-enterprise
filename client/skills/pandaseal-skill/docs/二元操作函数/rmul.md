...
Reverse element-wise multiplication: `other * CipherSeries` or `other * CipherDataFrame`. Supports `fill_value` when either side has missing data.
...
- **other** — `CipherSeries`, `CipherArray`, or scalar: left-hand factor.
...
`CipherSeries` or `CipherDataFrame`: element-wise product (`other * self`).
...
out = cipher_df.rmul(2)
# same decrypted result as mul(2)
```
