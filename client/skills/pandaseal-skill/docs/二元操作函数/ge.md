...
Element-wise **greater-than-or-equal** comparison between a `CipherSeries` or `CipherDataFrame` and `other`. Equivalent to operators `>=` where supported.
...
mask = cipher_df.ge(150)
# Decrypted truth table: cost T,T,F; revenue F,T,T
```
