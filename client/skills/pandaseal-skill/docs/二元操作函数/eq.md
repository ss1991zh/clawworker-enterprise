...
Element-wise **equality** comparison between a `CipherSeries` or `CipherDataFrame` and `other`. Equivalent to operators `==` where supported.
...
mask = cipher_df.eq(150)
# Decrypted truth table: cost F,T,F; revenue F,F,F
```
