...
> 设计目标:多个用户通过本地 Agent 客户端使用大模型能力；Excel、CSV 与企业数据库数据全程加密、明文不出本机；Word/PDF/TXT 文档正文仅在用户明确确认后发送给所配置模型；
...
Excel、CSV 和企业数据库的结构化数据全程保持加密，大模型与中心主机看不到数据行。
若用户附加 Word、PDF、TXT 等文档，正文只在界面明确确认后发送给所配置的大模型。
...
3. **结构化数据零明文外泄** —— LLM 与中心主机只见 schema 与意图,不见数据行；文档正文采用单独的明确确认边界
